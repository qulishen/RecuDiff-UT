...
class RetinexFormer(nn.Module):
    def __init__(
        self, in_channels=3, out_channels=3, n_feat=31, stage=3, num_blocks=[1, 1, 1]
    ):
        ...
        self.body = nn.Sequential(*modules_body, nn.Sigmoid())
        # self.flare = nn.Sequential(
        #                 nn.Conv2d(in_channels=3, out_channels=6, kernel_size=1,bias=True),  # 使用1x1卷积层增加通道数
        #                 nn.Sigmoid()
        # )
    ...
    def forward(self, x):
        ...
        return out:[b,c,h,w]
        """
        # flare_and_img = self.flare(x)

        # flare = flare_and_img[:,3:,:,:]

        # img =  flare_and_img[:,:3,:,:]

        out = self.body(x)
        # out = torch.cat((out,flare),dim=1)
