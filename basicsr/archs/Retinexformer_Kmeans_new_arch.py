...
class FlareFormer_new(nn.Module):
    def __init__(
        self,
        in_channels=3,
        output_ch=6,
        ...
        **kwargs
        ...
        super(FlareFormer_new, self).__init__()
        self.dim = dim
        self.level = level
        ## input projection
    ...
    def forward(self, img):
        ...
        img_gray = torch.sum(img, dim=1, keepdim=True)

        for n_clusters in num_clusters:
            cluster_results = []

            for b in range(B):  # 对每个图像进行聚类
                img_flat = img_gray[b].view(-1, 1)  # 将单个图像展平为 (H * W, C)

                # 随机初始化中心
                centers = img_flat[torch.randint(0, img_flat.size(0), (n_clusters,))]

                for _ in range(10):  # 迭代次数
                    # 计算每个点到中心的距离
                    distances = torch.cdist(img_flat, centers.unsqueeze(0))
                    labels = torch.argmin(
                        distances, dim=2
                    )  # 改为 dim=2，因为 distances 的形状是 [1, 262144, 2]
                    ...
                    new_centers = torch.stack(
                        [img_flat[labels == i].mean(dim=0) for i in range(n_clusters)]
                    )
                ...
                # 使用最终的标签和中心创建图像
                dst = centers[labels].view(H, W, 1).to(torch.uint8)  # 转换为 uint8 格式
                cluster_results.append(dst.permute(2, 0, 1))  # 转换为 (C, H, W) 格式

            kmeans_list.append(
                torch.stack(cluster_results)
            )  # 将每个图像的结果堆叠在一起
        ...
        for i, (IGAB, FeaDownSample, KmeansDown) in enumerate(self.encoder_layers):
            kmeans_fea = KmeansDown(
                torch.cat((img, kmeans_list[i].reshape(B, 1, H, W)), dim=1)
            )
        ...
        kmeans_fea = self.bottleneck_kmeans(
            torch.cat((img, kmeans_list[-1].reshape(B, 1, H, W)), dim=1)
        )
